  ...
  try {
    ...
    const calculateDeposite = [
      ...
      },
      {
        ...
        "Transaction id":result.filter((item) => item.Currency === "VND").reduce((total,current)=> {return total+=Number(current.Amount)},0),
      ...
      },
      {
        ...
        "Card No": result.filter((item) => item.Currency === "CNY").reduce((total,current)=> {return total+=Number(current.GST)+Number(current.Payin_charges)},0),
        Amount: "0",
        Payin_charges: "1",
        GST: null,
        Currency: null,
